...
public class NotificationService extends Service {
    ...
    public synchronized void onDestroy() {
        Log.d(TAG, "onDestroy");
        if (ZRAMToolApp.bLog) ZRAMToolApp.mLogHelper.appendLog("NotificationService::onDestroy()", LogHelper.LogColor.GRAY);
        super.onDestroy();
...
//            remoteViews.setOnClickPendingIntent(R.id.buttonCleanAll, buttonIntent);
    ...
    private void cleanMemoryKeepingRecents() {
        ...
        if (ZRAMToolApp.bLog)
            ZRAMToolApp.mLogHelper.appendLog("recentCount= " + recentCount + " ..... Process limit= " + ZRAMToolApp.iProcessLimit, LogHelper.LogColor.RED);
        ...
        for (int i = 0; i < procCount; i++) {
            ...
            for (int iRec = 0; iRec < ZRAMToolApp.iProcessLimit && iRec < recentCount; iRec++) {
                ...
                if (sRecentPackageName.equals(sProcName)) {
                    ...
                    if (ZRAMToolApp.bLog) ZRAMToolApp.mLogHelper.appendLog("NOT KILL " + sProcName, LogHelper.LogColor.GREEN);

                }
            }
            if (bProcIsInRecentLimit == false) {
                activityManager.killBackgroundProcesses(sProcName);
                if (ZRAMToolApp.bLog) ZRAMToolApp.mLogHelper.appendLog(sProcName, LogHelper.LogColor.BLUE);
